  ...
  const MobileNav = () => (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-slate-200 px-6 py-3 flex items-center justify-between z-[60] shadow-[0_-10px_30px_-15px_rgba(0,0,0,0.1)]">
      ...
      <button 
        ...
        className={`flex flex-col items-center gap-1 transition-all ${view === 'about' ? 'text-indigo-600 scale-110' : 'text-slate-400'}`}
      >
      ...
      <div className="relative -mt-10">
        <button 
          ...
          className={`w-14 h-14 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-xl shadow-indigo-200 border-4 border-white transition-transform active:scale-90 ${view === 'submit' ? 'bg-indigo-700' : ''}`}
        >
      ...
      <button 
        ...
        className={`flex flex-col items-center gap-1 transition-all ${view === 'messages' ? 'text-indigo-600 scale-110' : 'text-slate-400'}`}
      >
      ...
      <button 
        ...
        className={`flex flex-col items-center gap-1 transition-all ${view === 'profile' ? 'text-indigo-600 scale-110' : 'text-slate-400'}`}
      >
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F9FAFC] selection:bg-indigo-100 selection:text-indigo-700">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 md:py-12 w-full flex-1 mb-20 md:mb-0">
            ...
            <form onSubmit={(e) => {
              ...
              <div>
                ...
                <input
                  ...
                  name="email"
                  type="email"
                  placeholder="name@aauekpoma.edu.ng"
                  className="w-full px-6 py-5 bg-slate-50 border border-slate-200 rounded-[1.5rem] focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 focus:bg-white outline-none transition-all font-bold text-slate-900 placeholder:text-slate-300"
                />
              </div>
              <button
                type="submit"
                className="w-full bg-slate-900 text-white font-black py-5 rounded-[1.5rem] shadow-2xl shadow-indigo-100 hover:bg-indigo-600 transition-all active:scale-[0.98] uppercase tracking-[0.2em] text-[11px] flex items-center justify-center gap-3 group"
              >
              ...
              <div className="pt-6 border-t border-slate-50 text-center">
                <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.1em] leading-relaxed">
                  Only authorized @aauekpoma.edu.ng <br /> identities are permitted.
                </p>
              </div>
            </form>
          </div>
        ...
        {view === 'about' && (
          <AboutPage onGetStarted={() => navigateTo(user ? 'home' : 'login')} />
        )}
        ...
        {view === 'profile' && user && (
          <ProfilePage 
            ...
            onNavigateToInbox={() => navigateTo('messages')}
            onViewPostDetails={(post) => setDetailedPost(post)}
          />
        )}
          ...
          <div className="space-y-12">
            <div className="flex flex-col lg:flex-row gap-8 items-start lg:items-end justify-between">
              <div>
                ...
                <p className="text-slate-500 text-lg mt-3 font-medium">Verify property with AAU Security & Office of Student Affairs.</p>
              </div>
              <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
                ...
                <div className="relative flex-1 sm:flex-none">
                  <input 
                    ...
                    className="pl-12 pr-6 py-4 bg-white border border-slate-200 rounded-2xl shadow-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none w-full sm:w-80 transition-all font-bold placeholder:text-slate-300"
                    value={filter.search}
            ...
            {filteredPosts.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {filteredPosts.map(post => (
                  <div 
                    ...
                    className="group bg-white rounded-[2.5rem] shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100 overflow-hidden flex flex-col h-full cursor-pointer"
                  >
                    <div className="relative h-56 overflow-hidden bg-slate-50">
                      ...
                      <div className={`absolute top-4 left-4 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] shadow-xl ${post.type === PostType.LOST ? 'bg-rose-500 text-white' : 'bg-emerald-500 text-white'}`}>
                        {post.type}
                      </div>
                    </div>
                    <div className="p-8 flex-1 flex flex-col">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">{new Date(post.date).toLocaleDateString()}</span>
                      </div>
                      <h3 className="text-xl font-black text-slate-900 mb-2 line-clamp-1 group-hover:text-indigo-600 transition-colors tracking-tight">{post.title}</h3>
                      <p className="text-sm text-slate-500 line-clamp-2 mb-8 flex-1 leading-relaxed font-medium">{post.description}</p>
                      <div className="flex items-center gap-2 mb-8 text-[10px] font-black text-slate-400 bg-slate-50 p-3 rounded-xl uppercase tracking-widest">
                      ...
                      <button 
                        ...
                        className="w-full bg-slate-900 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] transition-all hover:bg-indigo-600 active:scale-95 shadow-xl shadow-slate-100 flex items-center justify-center gap-2"
                      >
                        Initiate Claim
                      </button>
                    </div>
              ...
              <div className="py-32 flex flex-col items-center text-center bg-white rounded-[3rem] border border-slate-100 shadow-sm px-6">
                ...
                <p className="text-slate-400 max-w-sm mt-3 text-lg font-medium">No reports match your current search. New submissions require security review before listing.</p>
              </div>
            )}
          </div>
        )}
        ...
        {view === 'admin' && user?.role === UserRole.ADMIN && (
          <div className="space-y-8">
            <div>
              ...
              <p className="text-slate-500 text-lg mt-1 font-medium">Manage and moderate campus property reports.</p>
            </div>
            <AdminPanel 
              posts={posts} 
              onAction={handlePostAction} 
              onDelete={handleDeletePost}
              onViewDetails={(post) => setDetailedPost(post)}
            />
          </div>
        )}
      </main>
      ...
      {detailedPost && (
        <PostDetailModal 
          post={detailedPost} 
          currentUser={user || { id: '', name: 'Guest', email: '', role: UserRole.USER }} 
      ...
      <footer className="mt-auto py-12 md:py-24 text-center text-slate-400 text-[9px] font-black uppercase tracking-[0.5em] bg-white border-t border-slate-100 mb-16 md:mb-0">
        <p>&copy; {new Date().getFullYear()} Ambrose Alli University (AAU) Property Office</p>
        <p className="mt-4 opacity-40">Verified Ekpoma Registry Portal</p>
      </footer>
